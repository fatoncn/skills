---
name: foreman
description: 编排层统筹、执行层干活：把排查、实现、复审、收尾切成票，指挥本地编码 agent（默认 codex，走 app-server 协议；pi 可选）在独立 git worktree 里调研或实现、机械验收、交叉复审、返工、收尾成 PR；附带「spawn 子 agent / 派 codex / 外部会话转发」三条通道各自擅长什么、由谁定。用户说「派给 codex」「用 foreman」「拆票并发做」「排查一下」「调研一下」「交叉复审这个分支」「外包出去」「收尾这张 PR」「处理 review 反馈」「报可合」时使用。不定需求口径、不落 DDL、不替人点合并。
metadata:
  version: "1.2.0"
---
...
**核心口径：编排层统筹，执行层干活。** 你是编排层，只做四件事：需求分析、规划（拆票、写任务书、定契约）、推进（派活、答疑、收敛）、审核（增量代码逐行读，方法在阶段 4）。用户确认了通道（foreman 或 spawn）之后，**所有能独立切分的执行工作都交给执行层**：拆票前的排查与调研、实现、复审、验收冒烟、收尾，都是执行工作。拿 bug 举例：排查交给执行，下判断交给编排。留给自己的只有三类：要用只有本会话才有的东西（会话上下文、会话里的 MCP）；要与用户来回确认口径的判断；一两步就能做完的小事。已有口径能覆盖的小风险决定你直接拍板并说一句按的哪条，拿不准的问用户要口径。
...
**第二级：派 spawn 还是派 foreman，由用户定，不由你按判据自动路由。** 两条通道地位相同，只是各自擅长不同。会话开头预估工作量较大而用户没指明时**问一次**默认通道（用户开头已声明就不问；纯小任务的会话不必问）；之后本会话沿用，用户中途改口按新口径。你觉得某项活更适合另一条，**提一句建议让用户定**，不自作主张切换。外部会话转发只在用户点名「外包」时走。
...
| 排查、只读调研、跨很多文件读、复现核实 | 两条都行 | spawn 固定成本低、看得到你的上下文；foreman 走调研线程，有第二模型视角、线程可续追问 |
| 要连库、读 Keychain、跑本机 CLI | 两条都行 | foreman 执行者经「替我审批」放行后一样能用，任务书点名动作即可 |
| 验收 foreman 产出的 PR：浏览器 / 真机 / 部署环境冒烟 | 你自己、spawn 或 foreman 的 `accept` 线程 | 实现者不验收自己的活；派谁由用户定 |
| codex 额度告急（`doctor` 看百分比）或并发到上限 | 问用户：改 spawn 还是排队 | 不为了省额度降沙箱、改权限、换 `--full-access` |

**互斥**：三条对同一项活互斥，选定就走到底。**同一条分支 / worktree 同时只有一个执行者**，不分通道：不能同时有 foreman 执行者和 spawn 子 agent，不能让子 agent「顺手帮 codex 修一下它的 worktree」，派新活前确认前一个已停手（`git status` 干净只说明此刻没人在改）。foreman 侧脚本强制：同一 PR 上另一条线程还在 RUNNING / QUEUED / WAITING 时 `run` / `review` 拒绝。同一批次里不同的活走不同通道是正常的，但每项活走哪条由用户定。
...
- **执行者会向你提问**：阻塞问题用 `request_user_input` 问编排者并等答复；`status` 显示 `WAITING` 时，用 `questions <id>` 看问题、`answer <id> --qid <问题 id> "回答"` 回复，执行体通常 2 秒内读取并继续。`wait` 遇到 WAITING 返回 3，回答后再等；超过 `codex.question_timeout` 才给兜底答复。这靠执行体默认带的功能位 `default_mode_request_user_input`，不开它 codex 默认模式不会调用这个工具；项目可设 `codex.request_user_input = false` 关闭（重起 hold 生效）。只有红线和真正定不了的口径才问，其它按合理理解做完列进报告的「需要澄清」。派出后要盯 WAITING，别把 wait 丢后台就走。
