---
name: foreman
description: 编排层统筹、执行层干活。一份分工指导 + 一套外派框架。分工指导（统筹与执行怎么切、三条通道 foreman 派 codex → 宿主 spawn 子 agent → 外部会话转发 怎么选、任务书与验收怎么写怎么核）任何编排会话都适用，走 spawn 或外部会话时同样按它；外派框架（切票，派 codex 主线 / claude beta 在独立 git worktree 里调研或实现、机械验收、交叉复审、返工、收尾成 PR，pi 可选）只在要把活外派给 spawn 之外的 Agent 时启用，也只在那时才需要初始化。用户说「派给 codex / claude」「用 foreman」「拆票并发做」「排查一下」「调研一下」「交叉复审这个分支」「外包出去」「收尾这张 PR」「处理 review 反馈」「报可合」，或要 spawn 子 agent 分工时使用。不定需求口径、不落 DDL、不替人点合并。
metadata:
  version: "1.5.1"
---
...
这一节只在要把活外派给 spawn 之外的 Agent（codex / claude / pi 执行器）时才需要做。没做完，受影响的只有 `$FOREMAN` 命令（`run` / `review` 拒绝派发）：分工判断、任务书、验收纪律照用，通道按「分工」第二级退到 spawn；告知用户 foreman 尚未初始化，要不要现在初始化由用户定，不替用户跑 `setup --confirm`。
...
**第二级：通道按优先级选，用户明确指定的分工优先。** 执行工作优先外派 foreman；foreman 无法外派或外派做不成——需要编排层独有能力（会话上下文、编排者会话里的 MCP、宿主自己的工具）、执行器不可用、或本机还没做完「第一次用」——再考虑 spawn；最后才是交接外部 Agent（用户点名「外包」，或前两条都走不通）。用户明确要求某项活怎么分工（「这个用 spawn」「这批外包」）就按用户的，本会话沿用，中途改口按新口径；改道说一句按的哪条。「活小」不是改道或自己做的理由——编排者自己动手的上限只有一条：已知文件或符号的一两条 grep、读一个文件（单点核对）；超过这个量就是调研，派出去。

| 通道 | 怎么派 | 它有什么 | 代价 |
|---|---|---|---|
| **foreman 派 codex**（本 skill 主线） | `$FOREMAN run` | 独立 worktree + OS 沙箱；线程可续、返工记得上一轮；第二模型视角；`report` 事后越界探针；本机凭证、CLI、网络经「替我审批」放行后都能用；调研走 `--role research` | 吃 codex 订阅额度（与用户自己的 Codex 共享）；一轮 20～40 分钟；看不到你的会话上下文和你的 MCP |
...
| **外部会话转发** | 写任务书 + 短 prompt，用户手动贴到外部 Agent | 不同 Agent 视角互补；人在环 | 交接靠人转述，一致性差 |

各自擅长（在上面的优先级之内参考，用户指定的优先）：

| 活的类型 | 更顺手的通道 | 为什么 |
|---|---|---|
| 产出代码进 PR、完成定义能机械判定、1～3 小时量 | foreman | worktree 隔离、线程可续返工、第二模型视角 |
| 排查、只读调研、跨很多文件读、复现核实 | foreman 调研线程（`--role research`）；活强依赖你的会话上下文才 spawn | foreman 有第二模型视角、线程可续追问；spawn 看得到你的上下文 |
| 要连库、读 Keychain、跑本机 CLI | foreman；只有要用编排者会话里的 MCP 才 spawn | foreman 执行者经「替我审批」放行后一样能用，任务书点名动作即可 |
| 验收 foreman 产出的 PR：浏览器 / 真机 / 部署环境冒烟 | foreman 的 `accept` 线程；要宿主独有工具（真机、会话里的 MCP）才 spawn 或你自己 | 实现者不验收自己的活 |
| codex 额度告急（`doctor` 看百分比）、并发到上限或执行器不可用 | 属「外派做不成」：告知用户，排队还是改 spawn 由用户定 | 不为了省额度降沙箱、改权限、换 `--full-access` |

**互斥**：三条对同一项活互斥，选定就走到底。**同一条分支 / worktree 同时只有一个执行者**，不分通道：不能同时有 foreman 执行者和 spawn 子 agent，不能让子 agent「顺手帮 codex 修一下它的 worktree」，派新活前确认前一个已停手（`git status` 干净只说明此刻没人在改）。foreman 侧脚本强制：同一 PR 上另一条线程还在 RUNNING / QUEUED / WAITING 时 `run` / `review` 拒绝。同一批次里不同的活走不同通道是正常的；每项活走哪条按第二级的优先级，用户指定的优先。
...
- 已派出去的活不重复再跑；需求或任务中途改了，在跑的线程不会自己看到，foreman 默认立刻 `steer` 通知（纠偏、撤回、下一轮预告都算），再用 `tail` 看到它确认并改了方向才算送达。只有变化与本轮无关、留到下一轮不会造成两轮改同一处，才用下一轮 `run`；工具阻塞提问用 `answer`。spawn 用 `SendMessage` 给同一个 agent，外部会话把增补 prompt 给用户转。
...
拆票前要核的事实：最新 base 上的现状与锚点（文件:行号）、数据形状（库里长什么样）、在途 PR 碰了哪些文件、能不能复现。这些是执行工作：够量（要翻十几个文件、要查库、要复现）就先派一张调研票，交付事实清单，你拿着它拆票；只有一两步的小核对自己看。调研线程的走法见阶段 3，任务书模板 `research-<id>.md` 见 `references/issue-pr-flow.md`。
...
**复审不判「需求做对没有」**——复审者没有你的上下文，判不了。它的活是**对抗性视角**：这次改动有没有破坏项目约定（项目根 `AGENTS.md`）、仓库约定（仓库内 `AGENTS.md`、周边代码的写法）、最佳实践；给的是**意见 + 依据**（哪条约定、哪个实践、什么输入会出错），不是判决，采不采纳由你逐条拍板。它最需要的上下文是你写的需求口径：为什么做、做到什么程度、哪些是有意不做的——没有这段它只能对着 diff 猜，把权衡后的边界当遗漏报。它顺带交一张「要求 → diff 位置」对照表，那是给你逐行审用的索引，不是结论。**按风险档决定跑不跑**：低风险（约 ≤50 行、不碰 prompt / 数据写路径 / 迁移 / 权限 / 计费 / 并发）跳过复审，靠你逐行审加测试 / CI；中高风险必跑。模板见 `references/issue-pr-flow.md`。
...
验收者按清单逐项操作、证据落交付目录、不在范围项只记不判、发现问题只报不修；交回来的是填好的 `ACCEPT-<PR>.md` 和逐项结果，可不可合由你判。派谁按第二级的优先级：默认 foreman `accept` 线程，要宿主独有工具才 spawn 或你自己；用户指定的优先。任务书模板 `accept-brief-<id>.md` 见 `references/issue-pr-flow.md`。
...
- PR 默认 draft（`github.pr_draft`），CI 绿、描述完整（含「复测入口」节、风险档；项目要求的实测证据节按项目规则）后再收尾转 Ready。收尾由实现者续同一线程（`run --closeout`），外部会话模式下则把收尾任务书贴给做实现的那个会话。
