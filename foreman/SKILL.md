...
**第二级：通道按优先级选，用户明确指定的分工优先。** 执行工作优先外派 foreman；foreman 无法外派或外派做不成——需要编排层独有能力（会话上下文、编排者会话里的 MCP、宿主自己的工具）、或执行器不可用——再考虑 spawn；最后才是交接外部 Agent（用户点名「外包」，或前两条都走不通）。用户明确要求某项活怎么分工（「这个用 spawn」「这批外包」）就按用户的，本会话沿用，中途改口按新口径；改道说一句按的哪条。「活小」不是改道或自己做的理由——编排者自己动手的上限只有一条：已知文件或符号的一两条 grep、读一个文件（单点核对）；超过这个量就是调研，派出去。
...
| **宿主 spawn 子 agent** | Claude Code `Agent` 工具（默认次旗舰模型，如 `opus`） | 快、便宜；看得到你的上下文与工具；追加指令用 `SendMessage` 续同一个 agent | 同源模型视角单一；没有 OS 沙箱，只有权限模式 |
...
- 收敛用 `status` / `wait --timeout <秒>`（返回 2 = 还在跑再调一次；3 = 执行者在提问，问题已打出，`answer` 后再 wait）/ `tail <id>`（跑到一半看进度）。**`wait` 会阻塞调用它的会话，强烈不建议前台等**：它挂着的时候你回不了用户、派不了别的票、答不了别的票的提问（执行体是独立进程，不受影响）。宿主能把命令放后台并在退出时唤醒你的（如 Claude Code 的 Bash `run_in_background`），一律放后台跑，`--timeout` 取宿主允许的上限，返回 2 就再放一次；是不是后台由宿主决定，skill 管不了，所以不限 timeout。`DEAD` = 进程没了但没写完成标记，按失败处理。`THREAD_BUSY` = shared 模式下桌面端正打开着这条线程、续不上（这轮什么都没跑）：让用户关掉它再续，急就 `release` 后 `--thread <新名>` 另起并在 prompt 里补上下文。
...
**复审不判「需求做对没有」**——复审者没有你的上下文，判不了。它的活是**对抗性视角**：这次改动有没有破坏项目约定（项目根 `AGENTS.md`）、仓库约定（仓库内 `AGENTS.md`、周边代码的写法）、最佳实践；给的是**意见 + 依据**（哪条约定、哪个实践、什么输入会出错），不是判决，采不采纳由你逐条拍板。它最需要的上下文是你写的需求口径：为什么做、做到什么程度、哪些是有意不做的——没有这段它只能对着 diff 猜，把权衡后的边界当遗漏报。它顺带交一张「要求 → diff 位置」对照表，那是给你逐行审用的索引，不是结论。**按风险档决定跑不跑**：低风险（约 ≤50 行、不碰 prompt / 数据写路径 / 迁移 / 权限 / 计费 / 并发）跳过复审，靠你逐行审加 accept 冒烟；中高风险必跑。模板见 `references/issue-pr-flow.md`。
...
**验收（用户面冒烟）走 `accept` 线程**：机械与代码审过之后，把产品真跑起来对着清单看的活派出去：
...
- **执行者不等 CI**：收尾轮 push 并回复 / resolve 本轮线程后立即交报告结束，不等待这次 push 的 CI 或下一轮 review。编排者在宿主后台用 `wait` 机制配合 `gh pr checks` / `gh run view` 轮询当前 head；CI 红了或出现新 finding，再派下一轮收尾，不让执行者 turn 前台空等。
...
- **小修轻量收尾**：改动约 ≤50 行、不改 prompt 的修复，不做全范围复审、不写闭环表格、不重复 preview 自证；push 后一句话报 head 与一条证据，真实复测交给合测做一次。验收后追加的小改动不重跑整套冒烟，收尾复测只做相关项。
- **批次合测只做集成态冒烟**：执行者的开发自测（单测、类型检查、prompt 抽样、本 PR 触碰的面）回答「改动做到了它说的事」；合测回答「用户流程走得通、没有非预期问题」，两层不互相替代。合测只核「合并后每个 PR 的效果都在、受影响主流程走通一遍、同一片代码没互相打坏」，不重跑执行者已自证的项；只有回归和流程走不通判阻塞，批次外既有问题只记不判。合测任务书按此写，不复制单 PR 的验收清单。
