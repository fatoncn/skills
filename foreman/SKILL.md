...
- **返工复测按改动性质分两档**（cookie 09-12）：验收通过后的返工轮不重跑整套冒烟，但**用户可见的修复项**（页面、导航、交互、用户能看到的数据与文案）要在报可合前补一次**增量 Preview 冒烟**——只点被改的那几条路径（十分钟级），多轮返工的可见项攒到最后一轮一起点，派 `accept` 线程或并入合测；纯 API / 内部行为的返工靠钉住该行为的单测 + CI，不上浏览器。
