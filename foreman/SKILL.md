...
**复审不判「需求做对没有」**——复审者没有你的上下文，判不了。它的活是**对抗性视角**：这次改动有没有破坏项目约定（项目根 `AGENTS.md`）、仓库约定（仓库内 `AGENTS.md`、周边代码的写法）、最佳实践；给的是**意见 + 依据**（哪条约定、哪个实践、什么输入会出错），不是判决，采不采纳由你逐条拍板。它最需要的上下文是你写的需求口径，**给得越多视角越全**：为什么做、做到什么程度、哪些是有意不做的，再加用户原话（带日期）、会话里定过的口径与否决过的方案、相关 issue / 文档链接、批次背景（复审线程不自动注入 `--context` 的背景文档，要么贴路径要么摘要点）、上一轮复审采纳 / 不采纳的决定——没有这些它只能对着 diff 猜，把权衡后的边界当遗漏报。它自动拿到的是角色文件、上一轮任务书、执行者交付报告和完整 diff，这些不用再贴。它顺带交一张「要求 → diff 位置」对照表，那是给你逐行审用的索引，不是结论。**按风险档决定跑不跑**：低风险（约 ≤50 行、不碰 prompt / 数据写路径 / 迁移 / 权限 / 计费 / 并发）跳过复审，靠你逐行审加测试 / CI；中高风险必跑。模板见 `references/issue-pr-flow.md`。
