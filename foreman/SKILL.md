---
name: foreman
description: 编排层统筹、执行层干活。一份分工指导 + 一套外派框架。分工指导（统筹与执行怎么切、两条通道 外派 foreman / 宿主 spawn 子 agent 怎么选、任务书与验收怎么写怎么核）任何编排会话都适用，走 spawn 时同样按它；外派框架（切票，派 codex 主线 / claude beta 在独立 git worktree 里调研或实现、机械验收、交叉复审、返工、收尾成 PR，pi 可选）只在要把活外派给 spawn 之外的 Agent 时启用，也只在那时才需要初始化。用户说「派给 codex / claude」「用 foreman」「拆票并发做」「排查一下」「调研一下」「交叉复审这个分支」「收尾这张 PR」「处理 review 反馈」「报可合」，或要 spawn 子 agent 分工时使用。不定需求口径、不落 DDL、不替人点合并。
metadata:
  version: "1.5.3"
---
...
**第二级：通道按优先级选，用户明确指定的分工优先。** 执行工作优先外派 foreman；foreman 无法外派或外派做不成——需要编排层独有能力（会话上下文、编排者会话里的 MCP、宿主自己的工具）、执行器不可用、或本机还没做完「第一次用」——再考虑 spawn；只有这两条通道。用户明确要求某项活怎么分工（「这个用 spawn」「这个派 codex」）就按用户的，本会话沿用，中途改口按新口径；改道说一句按的哪条。「活小」不是改道或自己做的理由——编排者自己动手的上限只有一条：已知文件或符号的一两条 grep、读一个文件（单点核对）；超过这个量就是调研，派出去。
...
**互斥**：两条对同一项活互斥，选定就走到底。**同一条分支 / worktree 同时只有一个执行者**，不分通道：不能同时有 foreman 执行者和 spawn 子 agent，不能让子 agent「顺手帮 codex 修一下它的 worktree」，派新活前确认前一个已停手（`git status` 干净只说明此刻没人在改）。foreman 侧脚本强制：同一 PR 上另一条线程还在 RUNNING / QUEUED / WAITING 时 `run` / `review` 拒绝。同一批次里不同的活走不同通道是正常的；每项活走哪条按第二级的优先级，用户指定的优先。
