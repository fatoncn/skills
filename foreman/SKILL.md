...
**第二级：派 spawn 还是派 foreman，由用户定，不由你按判据自动路由。** 两条通道地位相同，只是各自擅长不同。会话开头预估工作量较大而用户没指明时**问一次**默认通道（用户开头已声明就不问；纯小任务的会话不必问）；之后本会话沿用，用户中途改口按新口径。你觉得某项活更适合另一条，**提一句建议让用户定**，不自作主张切换。外部会话转发只在用户点名「外包」时走。**用户定下的通道对本会话全部执行工作生效，调研、排查也在内**；「活小」不是改道或自己做的理由——编排者自己动手的上限只有一条：已知文件或符号的一两条 grep、读一个文件（单点核对）；超过这个量就是调研，派出去。改道先问，不问就是违反。
...
| 排查、只读调研、跨很多文件读、复现核实 | 按用户定的通道；没定才两条都行 | spawn 固定成本低、看得到你的上下文；foreman 走调研线程，有第二模型视角、线程可续追问 |
| 要连库、读 Keychain、跑本机 CLI | 按用户定的通道；没定才两条都行 | foreman 执行者经「替我审批」放行后一样能用，任务书点名动作即可 |
