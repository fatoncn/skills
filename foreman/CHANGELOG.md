...
- Codex `research` 默认档位由 gpt-6-astra / medium 恢复为 gpt-5.6-sol / high，降低调研的旗舰模型消耗；Claude `research` 档位及其他角色不变。

## 1.5.11 — 2026-09-20

- review 机器人反馈到达默认 1～2 轮上限后交用户拍板，不自行进入第 3 轮；用户明确开启第 3 轮时保持最大克制，只处理严重影响功能口径、触发概率高、严重影响用户体验的缺陷，其余回复并 resolve，不改代码、不顺带重构。
- Codex `research` 默认档位由 gpt-5.6-sol / high 改为 gpt-6-astra / medium；Claude `research` 档位及其他角色不变。

## 1.5.10 — 2026-09-19

- 收窄 Foreman 技能触发条件：明确要求 Foreman 外派或管理已有 Foreman 票、线程时使用，普通调研、排查和代码审查不再自动触发。
- 将 `SKILL.md` 从 326 行精简为 21 行的按需文档入口；通用分工、通道选择、角色档位、任务书、审核、验收与收尾指导移至 `references/orchestration-guide.md`。
- 将初始化、权限边界、对象模型、派发、线程生命周期、命令速查与执行器限制移至 `references/foreman-operations.md`；仅使用宿主子 agent 或查编排指导时无需加载外派操作手册或初始化 Foreman。
- 更新任务书模板中的指导引用。保留既有指导策略及全部 9 个 Bash 命令块，本次不改执行器脚本。

## 1.5.9 — 2026-09-17

- `cfg` / `cfg_opt` 在没有项目配置文件时回落全局配置，再回落调用方默认值。
- `doctor` 握手只读全局层，不再依赖 git 仓库或项目配置（修 #31）。
- app-server 握手失败时追加 stderr 尾部，便于区分登录、额度、参数和进程崩溃。
